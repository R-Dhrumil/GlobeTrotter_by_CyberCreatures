      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2.5 rounded-xl border border-stone-300 text-sm text-stone-900 focus:outline-none focus:border-amber-600 font-medium"
          />
        </div>
        ...
        <div className="p-4 rounded-2xl bg-stone-50 border border-stone-200 space-y-3">
          <div className="flex items-center justify-between">
            ...
            {durationInfo && (
              <span className="px-2.5 py-1 rounded-lg bg-amber-100 text-amber-900 text-[11px] font-bold flex items-center gap-1 border border-amber-300">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-xl border border-stone-300 text-xs text-stone-900 focus:outline-none focus:border-amber-600 bg-white"
              />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-stone-600 mb-1">
          ...
          {formData.startDate && (
            <div className="flex items-center gap-2 pt-1">
              <span className="text-[10px] text-stone-500 font-semibold">Quick Duration:</span>
              {[3, 5, 7, 10, 14].map((days) => (
                <button
                  key={days}
                  type="button"
                  onClick={() => handleDurationDaysChange(days)}
                  className="px-2 py-0.5 rounded-md bg-white hover:bg-amber-100 text-stone-700 text-[10px] font-bold border border-stone-200 transition"
                >
                  {days} Days
                </button>
              ))}
        ...
        <div className="p-4 rounded-2xl bg-emerald-50/50 border border-emerald-200/80 space-y-3">
          <div className="flex items-center justify-between">
            ...
            {perPersonSplit && (
              <span className="px-2.5 py-1 rounded-lg bg-emerald-100 text-emerald-900 text-[11px] font-bold border border-emerald-300 flex items-center gap-1">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-xl border border-stone-300 text-xs text-stone-900 focus:outline-none focus:border-emerald-600 bg-white"
              />
            </div>
          ...
          {perPersonSplit && (
            <div className="p-2.5 rounded-xl bg-white border border-emerald-200 text-xs text-stone-700 flex items-center justify-between">
              <span className="text-[11px] text-stone-600">
                Split Calculation: <strong>${perPersonSplit.budget.toLocaleString()} total</strong> ÷ <strong>{perPersonSplit.count} traveler(s)</strong>
              </span>
          ...
          <div className="grid grid-cols-3 gap-2 mb-2">
            {sampleCoverPhotos.map((photo, i) => (
              ...
              >
                ...
                <span className="absolute bottom-1 left-1 text-[9px] bg-black/60 text-white px-1.5 py-0.5 rounded backdrop-blur-xs truncate max-w-[90%] font-medium">
                  {photo.label}
                </span>
              </div>
            ))}
        ...
        <div className="pt-4 flex items-center justify-end gap-3 border-t border-stone-100">
          ...
          <button
            type="submit"
            disabled={loading}
            className="px-6 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs shadow-md transition disabled:opacity-50 flex items-center gap-2"
          >
